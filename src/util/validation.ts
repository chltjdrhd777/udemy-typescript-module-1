namespace App {
  ...
  export function validate(validatableInput: Validatable) {
    ...
    if (validatableInput.required) {
      ...
      isValid =
        isValid && validatableInput.value.toString().trim().length !== 0; // new isValid's answer is up to the following condition. if validatableInput.value(is string) is not empty, it is true.
    }
    if (
